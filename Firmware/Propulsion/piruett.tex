...
\Y\B\\{cli}(\,);\7
\X50:Initialize the inputs and capture mode\X\X52:Initialize tick timer\X%
\X47:Initialize pin outputs\X\X54:Initialize watchdog timer\X\Y\par
\fi
...
OC0B (D6), respectivly.
\Y\B\X56:Initialize the Timer Counter 0 for PWM\X\par
\fi
...
\Y\B\X48:Configure to idle on sleep\X\7
\\{ledCntl}(\.{OFF});\par
\fi
...
\&{if} ${}(\R(\PP\\{tickCount}\AND\CM\T{\^80}){}$)\SHC{ every 128 ticks }\6
${}\{\,\}{}$\7
\4${}\}{}$\2\Y\par
\fi
...
\M{47}\B\X47:Initialize pin outputs\X${}\E{}$\SHC{ set the led port direction;
This is pin \#17 }\6
...
\M{48}\B\X48:Configure to idle on sleep\X${}\E{}$\6
${}\{{}$\1\6
...
\M{50}\B\X50:Initialize the inputs and capture mode\X${}\E{}$\6
...
\M{52}\B\X52:Initialize tick timer\X${}\E{}$\6
${}\{{}$\1\6
${}\.{TCCR2B}\MRL{{\OR}{\K}}(\T{1}\LL\.{CS22})\OR(\T{1}\LL\.{CS21})\OR(\T{1}\LL%
\.{CS20}){}$;\SHC{ maximum prescale (see 18.11.2) }\6
${}\.{TCCR2A}\MRL{{\OR}{\K}}(\T{1}\LL\.{WGM21}){}$;\SHC{ CTC mode (see 18.11.1)
}\6
${}\.{OCR2A}\K\T{\^f3}{}$;\SHC{ Do I need to make this clearer? }\6
${}\.{TIMSK2}\MRL{{\OR}{\K}}(\T{1}\LL\.{OCIE2A}){}$;\SHC{ Interrupt on a
compare match }\6
\4${}\}{}$\2\par
\U13.\fi
...
\M{56}\B\X56:Initialize the Timer Counter 0 for PWM\X${}\E{}$\6
